...
public class CandidateProfileController {
    ...
    @Operation(
            ...
            description = "Creates a new candidate profile in the system",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Candidate profile creation details",
                    required = true,
                    content = @Content(schema = @Schema(implementation = CandidateProfileCreateDTO.class))
            ),
            responses = {
                    @ApiResponse(responseCode = "201", description = "Candidate profile created successfully",
                            content = @Content(schema = @Schema(implementation = CandidateProfileDTO.class)))
            }
    )
    public ResponseEntity<APIResponse<CandidateProfileDTO>> create(@RequestBody CandidateProfileCreateDTO dto) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new APIResponse<>(true, "Candidate profile created successfully", result));
    }

    @PutMapping
    @Operation(
            summary = "Update Candidate Profile",
            description = "Updates an existing candidate profile based on its ID",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Updated candidate profile data",
                    required = true,
                    content = @Content(schema = @Schema(implementation = CandidateProfileUpdateDTO.class))
            ),
            responses = {
    ...
    @Operation(
            ...
            description = "Deletes a candidate profile based on the given ID",
            responses = {
    ...
    public ResponseEntity<APIResponse<Void>> delete(@PathVariable Integer id) {
        ...
        return ResponseEntity.ok(new APIResponse<>(true, "Candidate profile deleted successfully", null));
    }

    @GetMapping("/{id}")
    @Operation(
            summary = "Get Candidate Profile by ID",
            description = "Retrieves the candidate profile details for the given ID",
            responses = {
    ...
    public ResponseEntity<APIResponse<CandidateProfileDTO>> getById(@PathVariable Integer id) {
        ...
        return ResponseEntity.ok(new APIResponse<>(true, "Candidate profile retrieved successfully", dto));
    }

    @PostMapping("/filter")
    @Operation(
            summary = "Filter Candidate Profiles",
            description = "Returns a filtered list of candidate profiles based on various criteria such as name, email, skills, etc.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Filtering criteria for candidate profiles",
                    required = true,
                    content = @Content(schema = @Schema(implementation = CandidateProfileFilter.class))
            ),
            responses = {
                    @ApiResponse(responseCode = "200", description = "Candidate profiles filtered successfully",
                            content = @Content(array = @ArraySchema(schema = @Schema(implementation = CandidateProfileDTO.class))))
            }
    )
