...
public class ContractsController {
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    required = true,
                    content = @Content(schema = @Schema(implementation = ContractCreateDTO.class))
            ),
            responses = {
    ...
    public ResponseEntity<APIResponse<ContractDTO>> create(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new APIResponse<>(true, "Contract created successfully", result));
    }

    @PutMapping
    @Operation(
            summary = "Update Contract",
            description = "Updates an existing contract based on provided data",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Contract update details",
                    required = true,
                    content = @Content(schema = @Schema(implementation = ContractUpdateDTO.class))
            ),
            responses = {
    ...
    public ResponseEntity<APIResponse<ContractDTO>> update(
        ...
        return ResponseEntity.ok(new APIResponse<>(true, "Contract updated successfully", updated));
    }

    @DeleteMapping("/{id}")
    @Operation(
            summary = "Delete Contract",
            description = "Deletes the contract with the given ID",
            parameters = {
    ...
    public ResponseEntity<APIResponse<ContractDTO>> getById(@PathVariable Integer id) {
        ContractDTO dto = contractService.getById(id);
        return ResponseEntity.ok(new APIResponse<>(true, "Contract retrieved successfully", dto));
    }
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    required = true,
                    content = @Content(schema = @Schema(implementation = ContractFilter.class))
            ),
            parameters = {
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "Contracts filtered successfully",
